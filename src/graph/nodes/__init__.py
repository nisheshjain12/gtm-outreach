...
def draft(state: RunState) -> dict:
    # On a regeneration pass (looped back from draft_review) use the user's
    # regen instructions; otherwise the signal-review instructions.
    instructions = state.get("regen_instructions") or state.get("user_instructions", "")
    ...
    # Clear the regen flag so a later "approve" doesn't loop back here again.
    return {"drafts": result, "regen_instructions": None}


# ── 11. draft_review (interrupt) ─────────────────────────────────────────────

def draft_review(state: RunState) -> dict:
    """Single interrupt. The regeneration loop happens at the GRAPH level:
    when action == "edit" with instructions, a conditional edge routes back
    to the `draft` node to regenerate, then returns here to review the new
    draft. action == "approve" routes forward to `approve`.
    """
    choice = interrupt({
        "type": "draft_review",
    ...
    # A regenerate request only takes the loop if instructions were given.
    regenerate = action == "edit" and bool(instructions)
    ...
    return {
        ...
        "draft_action": "edit" if regenerate else "approve",
        "regen_instructions": instructions if regenerate else None,
    }
